#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <stdint.h>
//#include "xil_printf.h"
#include "key-expansion.h"
//#include "key-expansion.c"
#define n 4
#define CHECK_BIT(var, pos) (((var) >> (pos)) & 1)

void extractBytes(const char* input, int len, uint8_t **output);
void addRoundKey(uint8_t **plaintext, uint8_t **keys, int round);
void subBytes(uint8_t **plaintext);
void shiftRows(uint8_t **plaintext);
void rotateArray(uint8_t *arr, int d);
void reverse(uint8_t *arr, int start, int end);
void mixColumns(uint8_t **plaintext);
uint8_t mul(uint8_t a, uint8_t b);

// int main()
uint8_t** encryptAES(char* input)
{
    //uint8_t input[] = "Two One Nine two";
    uint8_t **plaintext = (uint8_t **)calloc(4, sizeof(uint8_t *));
    // if (plaintext==NULL) {
    //     xil_printf("calloc fucked!");
    //     return NULL;
    // }
    // xil_printf("Right after taking inside the function: %s\n\n",input);


    for (int i = 0; i < 4; i++)
    {
        plaintext[i] = (uint8_t *)calloc(4, sizeof(uint8_t));
    }

    int in_len = strlen(input);

    extractBytes(input, in_len, plaintext);

    uint8_t **keyset = generateKey();

    // xil_printf("Before encryption function: \n");
    
    // for (int i = 0; i < n; i++)
    // {
    //     for (int j = 0; j < n; j++)
    //     {
    //         xil_printf("%x ",plaintext[j][i]);
    //     }
    //     xil_printf("\n");
    // }


    addRoundKey(plaintext, keyset, 0);

    for (int round = 1; round <= 10; round++)
    {
        subBytes(plaintext);

        shiftRows(plaintext);

        if (round != 10)
        {
            mixColumns(plaintext);
        }

        addRoundKey(plaintext, keyset, round);

    }
    // xil_printf("After encryption function: \n");
    
    // for (int i = 0; i < n; i++)
    // {
    //     for (int j = 0; j < n; j++)
    //     {
    //         xil_printf("%x ",plaintext[j][i]);
    //     }
    // }

    return plaintext;
}

void extractBytes(const char* input, int len, uint8_t **output)
{
    int index = 0;

    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            if (index < len)
            {
                output[j][i] = input[index++];
            }
            else
            {
                output[j][i] = 0x00;
            }
        }
    }

    // xil_printf("After Extract Bytes: \n");
    
    // for (int i = 0; i < n; i++)
    // {
    //     for (int j = 0; j < n; j++)
    //     {
    //         xil_printf("%x ",output[j][i]);
    //     }
    // }

}

void addRoundKey(uint8_t **plaintext, uint8_t **keys, int round)
{

    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            plaintext[j][i] ^= keys[(round * 4) + i][j];
        }
    }
}

void subBytes(uint8_t **plaintext)
{
    // AES-128 Standard S-Box (Substitution Box)
    static const uint8_t S[16][16] = {
        };

    int x, y;
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            x = plaintext[i][j] / 16;
            y = plaintext[i][j] % 16;
            plaintext[i][j] = S[x][y];
        }
    }
}

void shiftRows(uint8_t **plaintext)
{
    rotateArray(plaintext[1], 1);
    rotateArray(plaintext[2], 2);
    rotateArray(plaintext[3], 3);
}

void rotateArray(uint8_t *arr, int d)
{
    d %= n;
    reverse(arr, 0, d - 1);
    reverse(arr, d, n - 1);
    reverse(arr, 0, n - 1);
}

void reverse(uint8_t *arr, int start, int end)
{
    while (start < end)
    {
        uint8_t temp = arr[start];
        arr[start] = arr[end];
        arr[end] = temp;
        start++;
        end--;
    }
}

void mixColumns(uint8_t **plaintext)
{
    // mixing columns like meth
    uint8_t temp[4][4];
    uint8_t fixed[4][4] = {2, 3, 1, 1, 1, 2, 3, 1, 1, 1, 2, 3, 3, 1, 1, 2};
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            temp[i][j] = 0;
            for (int k = 0; k < n; k++)
            {
                temp[i][j] = temp[i][j] ^ mul(fixed[i][k], plaintext[k][j]);
            }
        }
    }

    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < n; j++)
        {
            plaintext[i][j] = temp[i][j];
        }
    }
}

uint8_t mul(uint8_t fixed, uint8_t hex)
{
    uint8_t p = 0;
    uint8_t high_bit_set;
    for (int i = 0; i < 8; i++)
    {
        if (hex & 1)
            p ^= fixed;
        high_bit_set = (fixed & 0x80);
        fixed <<= 1;
        if (high_bit_set)
            fixed ^= 0x1b; // x^8 + x^4 + x^3 + x + 1
        hex >>= 1;
    }
    return p;
}
